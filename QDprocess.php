...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['resume']) && $_FILES['resume']['error'] === UPLOAD_ERR_OK) {
        ...
        $uploadDir = 'uploads/QD/'; // Relative path for public access
        ...
        if (move_uploaded_file($fileTmpPath, $serverFilePath)) {
            ...
            $stmt = $conn->prepare("INSERT INTO QD (name, email, phone, resume) VALUES (?, ?, ?, ?)");
            ...
            if ($stmt->execute()) {
                ...
                $fileURL = 'http://' . $_SERVER['HTTP_HOST'] . '/uploads/QD/' . urlencode($uniqueFileName);

                $message = "You have received a new Quantitative Developer form submission:\n\n";
                $message .= "Name: $name\n";
                $message .= "Email: $email\n";
                $message .= "Phone: $phone\n";
